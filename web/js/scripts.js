(function($){
	...
	$(document).ready(function () {
        ...
        $(".profile-nav #register").click(function (event) {
            event.preventDefault();
            $('.login-block').addClass('hidden');
            $('.register-block').removeClass('hidden');
        })

		$('[name="room_photos_form"]').submit(function (event) {
            event.preventDefault();
            var room = $(".room_form_hotel").val();
            var file = document.getElementById("room_photos_form_photo");
            var formdata=new FormData();
            for (var j = 0; j < file.files.length; j++) {
                formdata.append("file"+j, file.files[j]);
            }
            var fls = file.files.length;
            var anchor = "/uploadPhotos";
            formdata.append("room", room);
            formdata.append("fls", fls);
            var ajax = new XMLHttpRequest();
            ajax.open("POST", anchor);
            ajax.send(formdata);
        })

        $('#room_photos_form_room').change(function () {

            var option = $(this).val();
            var anchor = "/searchRooms";
            $(".room_form_hotel").val("");
            $.ajax({
                url: anchor,
                type: 'POST',
